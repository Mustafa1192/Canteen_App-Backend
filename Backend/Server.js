...
app.post('/api/send-otp', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            ...
            isNewUser: !user.username, // Determine if user is new
        });
    } catch (error) {
        console.error(error);
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            return res.status(404).json({ message: 'User not found. Redirecting to registration page.' });
        }

        if (user.otp === otp && user.otpExpiresAt > Date.now()) {
            user.username = username;
            user.password = password; // Ideally hash password using bcrypt
            user.otp = undefined;
            ...
            res.status(400).json({ message: 'Invalid or expired OTP. Please resend the OTP.' });
        }
    } catch (error) {
        console.error(error);
...
app.post('/api/resend-otp', async (req, res) => {
    ...
    try {
        ...
        const mailOptions = {
            from: process.env.EMAIL_USER,
            to: email,
            subject: 'Your OTP for Login/Registration (Resend)',
            text: `Your OTP is ${otp}. It is valid for 5 minutes.`,
        };
